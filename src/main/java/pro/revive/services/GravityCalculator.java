...
public class GravityCalculator {
    ...
    public static int calculateScore(Triage t) {
        ...
        if (pouls < 40 || pouls > 150) { score += 5; reasons.add("Pouls critique (" + pouls + " bpm)"); }
        ...
        if (temp < 35 || temp > 40) { score += 5; reasons.add("Temperature critique (" + temp + "C)"); }
        else if (temp < 36 || temp > 38) { score += 2; reasons.add("Temperature anormale (" + temp + "C)"); }

        // SpO2
        int spo2 = t.getSpo2();
        if (spo2 < 85) { score += 5; reasons.add("SpO2 critique (" + spo2 + "%)"); }
        else if (spo2 < 90) { score += 3; reasons.add("SpO2 bas (" + spo2 + "%)"); }
        else if (spo2 < 95) { score += 1; }

        // Glycemia
        float glycemie = t.getGlycemie();
        if (glycemie < 0.5f || glycemie > 3.0f) { score += 5; reasons.add("Glycemie critique (" + glycemie + " g/L)"); }
        else if (glycemie < 0.7f || glycemie > 2.0f) { score += 3; reasons.add("Glycemie anormale (" + glycemie + " g/L)"); }

        // Pain Score
        int douleur = t.getScoreDouleur();
        if (douleur >= 9) { score += 4; reasons.add("Douleur severe (" + douleur + "/10)"); }
        else if (douleur >= 7) { score += 3; reasons.add("Douleur importante (" + douleur + "/10)"); }
        else if (douleur >= 5) { score += 2; }

        // GCS
        int gcs = t.getGcsScore();
        if (gcs < 9) { score += 5; reasons.add("Conscience alteree GCS (" + gcs + "/15)"); }
        else if (gcs < 13) { score += 3; reasons.add("GCS reduit (" + gcs + "/15)"); }
        else if (gcs < 15) { score += 1; }

        // Respiratory Rate
        int freqResp = t.getFrequenceRespiratoire();
        if (freqResp < 10 || freqResp > 30) { score += 5; reasons.add("Frequence resp. critique (" + freqResp + "/min)"); }
        ...
        t.setAnalyseAuto(reasons.isEmpty()
